                    ...
                    <div class="flex items-center gap-3 min-w-0">
                        ...
                        <div class="min-w-0">
                            <h2 class="text-sm font-semibold text-zinc-900 dark:text-zinc-100 truncate">
                                @if($mode === 'view')
                                    {{ $name ?: 'MOF Category Details' }}
                                @elseif($mode === 'edit')
                                    Edit — {{ $name ?: 'MOF Category' }}
                                @else
                                    New MOF Category
                                @endif
                            </h2>
                            <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
                                ...
                                @elseif($mode === 'edit')
                                    Editing MOF category master code.
                                @else
                                    Add primary Ministry of Finance classification category.
                                @endif
                            </p>
                        </div>
                    </div>
                        ...
                        <div class="space-y-7">
                            <div>
                                ...
                                <dl class="grid grid-cols-2 gap-x-6 gap-y-4">
                                    <div>
                                        <dt class="text-xs font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">Category Code</dt>
                                        <dd class="mt-1 text-sm font-mono text-zinc-900 dark:text-zinc-100 font-semibold">{{ $code ?: '—' }}</dd>
                                    </div>
                                    <div class="col-span-2">
                                        <dt class="text-xs font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">Category Name</dt>
                    ...
                    @else
                        <form wire:submit.prevent="save" class="space-y-7">
                            <div>
                                ...
                                <div class="space-y-4">
                                    <div>
                                        <x-label for="mof_cat_code">Category Code *</x-label>
                                        <x-input id="mof_cat_code" wire:model="code" placeholder="e.g. 01" />
                                        @error('code') <p class="text-xs text-rose-600 dark:text-rose-400 mt-1">{{ $message }}</p> @enderror
                                    </div>

                                    <div>
                                        <x-label for="mof_cat_name">Category Name *</x-label>
                                        <x-input id="mof_cat_name" wire:model="name" placeholder="e.g. PERABOT, KELENGKAPAN DAN BEKALAN PEJABAT" />
