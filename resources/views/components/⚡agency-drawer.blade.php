...
new class extends Component
{
    ...
    public function open(string $mode = 'create', ?int $id = null): void
    {
        ...
        if ($id && in_array($mode, ['view', 'edit'])) {
            ...
            $this->is_active = (bool) $agency->is_active;
        }
    ...
    public function save(): void
    {
        ...
        if ($this->mode === 'edit' && $this->activeId) {
            $agency = Agency::findOrFail($this->activeId);
            ...
            <div x-show="open" x-transition:enter="transform transition ease-in-out duration-300" x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0"
                ...
                <div class="p-6 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
                    <div>
                        <h2 class="text-lg font-bold text-zinc-900 dark:text-zinc-100">
                            {{ $mode === 'create' ? 'Add New Agency' : ($mode === 'edit' ? 'Edit Agency' : 'Agency Details') }}
                        </h2>
                        <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">Government agency master record.</p>
                    </div>
                ...
                <div class="flex-1 overflow-y-auto p-6 space-y-4">
                    @if($mode === 'view')
                        <div class="space-y-4 text-sm">
                            <div>
                                <label class="text-xs font-medium text-zinc-500 dark:text-zinc-400">Agency Code</label>
                                <p class="font-semibold text-zinc-900 dark:text-zinc-100 mt-0.5">{{ $code }}</p>
                            </div>
                            <div>
                                <label class="text-xs font-medium text-zinc-500 dark:text-zinc-400">Agency Name</label>
                    ...
                    @else
                        <form wire:submit.prevent="save" class="space-y-4">
                            <div>
                                <x-ui.label for="agency_code">Agency Code *</x-ui.label>
                                <x-ui.input id="agency_code" wire:model="code" placeholder="e.g. AGY-1001" />
                                @error('code') <p class="text-xs text-rose-500 mt-1">{{ $message }}</p> @enderror
                            </div>

                            <div>
                                <x-ui.label for="agency_name">Agency Name *</x-ui.label>
                                <x-ui.input id="agency_name" wire:model="name" placeholder="e.g. Ministry of Communications" />
                                @error('name') <p class="text-xs text-rose-500 mt-1">{{ $message }}</p> @enderror
                            </div>

                            <div class="flex items-center gap-3 pt-2">
                                <input id="is_active" type="checkbox" wire:model="is_active" class="rounded border-zinc-300 text-emerald-600 focus:ring-emerald-500" />
                                <x-ui.label for="is_active" class="cursor-pointer mb-0">Active Status</x-ui.label>
                            </div>
                        </form>
                    @endif
                </div>
                ...
                <div class="p-6 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 flex justify-end gap-3">
                    @if($mode === 'view')
                        ...
                        <x-ui.button variant="primary" size="sm" wire:click="switchToEdit">Edit Agency</x-ui.button>
                    @else
                        <x-ui.button variant="outline" size="sm" wire:click="closePanel">Cancel</x-ui.button>
                        <x-ui.button variant="primary" size="sm" wire:click="save">Save Agency</x-ui.button>
                    @endif
                </div>
            </div>
        </div>
