...
<div class="space-y-6">
    ...
    <x-card>
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 mb-6">
            ...
            <x-button variant="primary" size="sm" @click="$dispatch('open-committee-drawer', { mode: 'create' })">
                <x-heroicon-o-plus class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" />
                Add New Committee
            </x-button>
        </div>

        <x-table :headers="['Committee Name', 'Slug', 'Configured Positions', 'Actions']">
                        ...
                        @if(is_array($committee->position) && count($committee->position) > 0)
                            <div class="flex flex-wrap gap-1">
                                @foreach(array_slice($committee->position, 0, 3) as $pos)
                                    <x-badge variant="info" pill>{{ $pos }}</x-badge>
                                @endforeach
                                @if(count($committee->position) > 3)
                                    <x-badge variant="secondary" pill>+{{ count($committee->position) - 3 }} more</x-badge>
                                @endif
                            </div>
                        @else
