            ...
            <div x-show="open" x-transition:enter="transform transition ease-in-out duration-300" x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0"
                ...
                <div class="p-6 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
                    <div>
                        <h2 class="text-lg font-bold text-zinc-900 dark:text-zinc-100">
                            {{ $mode === 'create' ? 'Add MOF Subcategory' : ($mode === 'edit' ? 'Edit Subcategory' : 'Subcategory Details') }}
                        </h2>
                        <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">Ministry of Finance subcategory master record.</p>
                    </div>
                ...
                <div class="flex-1 overflow-y-auto p-6 space-y-4">
                    @if($mode === 'view')
                        <div class="space-y-4 text-sm">
                            <div>
                                <label class="text-xs font-medium text-zinc-500 dark:text-zinc-400">Parent Category</label>
                                <p class="font-semibold text-zinc-900 dark:text-zinc-100 mt-0.5">
                                    {{ $this->categories->firstWhere('id', $mof_category_id)?->name ?? '-' }}
                                </p>
                            </div>
                            <div>
                                <label class="text-xs font-medium text-zinc-500 dark:text-zinc-400">Subcategory Code</label>
                                <p class="text-zinc-800 dark:text-zinc-200 mt-0.5">{{ $code }}</p>
                            </div>
                            <div>
                                <label class="text-xs font-medium text-zinc-500 dark:text-zinc-400">Subcategory Name</label>
                    ...
                    @else
                        <form wire:submit.prevent="save" class="space-y-4">
                            <div>
                                <x-ui.label for="mof_category_id">Parent MOF Category *</x-ui.label>
                                <select id="mof_category_id" wire:model="mof_category_id" class="w-full px-3 py-2 text-sm bg-zinc-50 dark:bg-zinc-800/60 border border-zinc-200 dark:border-zinc-700/80 rounded-xl text-zinc-900 dark:text-zinc-100">
                                    <option value="">Select Parent Category</option>
                                    @foreach($this->categories as $cat)
                                        <option value="{{ $cat->id }}">{{ $cat->name }}</option>
                                    @endforeach
                                </select>
                                @error('mof_category_id') <p class="text-xs text-rose-500 mt-1">{{ $message }}</p> @enderror
                            </div>

                            <div>
                                <x-ui.label for="sub_code">Subcategory Code *</x-ui.label>
                                <x-ui.input id="sub_code" wire:model="code" placeholder="e.g. 0101" />
                                @error('code') <p class="text-xs text-rose-500 mt-1">{{ $message }}</p> @enderror
                            </div>

                            <div>
                                <x-ui.label for="sub_name">Subcategory Name *</x-ui.label>
                                <x-ui.input id="sub_name" wire:model="name" placeholder="e.g. Perabot Rumah dan Pejabat" />
                ...
                <div class="p-6 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 flex justify-end gap-3">
                    @if($mode === 'view')
                        ...
                        <x-ui.button variant="primary" size="sm" wire:click="switchToEdit">Edit Subcategory</x-ui.button>
                    @else
                        <x-ui.button variant="outline" size="sm" wire:click="closePanel">Cancel</x-ui.button>
                        <x-ui.button variant="primary" size="sm" wire:click="save">Save Subcategory</x-ui.button>
                    @endif
                </div>
            </div>
        </div>
