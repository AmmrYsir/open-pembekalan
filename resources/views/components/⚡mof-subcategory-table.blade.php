...
new class extends Component
{
    ...
    {
        if ($this->deletingId) {
            MofSubcategory::findOrFail($this->deletingId)->delete();
            session()->flash('success', 'MOF Subcategory deleted successfully.');
        }
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <x-card>
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs text-zinc-500 dark:text-zinc-400 font-medium">Total Subcategories</p>
                ...
                <div class="w-10 h-10 rounded-xl bg-emerald-100 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 flex items-center justify-center">
                    <x-heroicon-o-folder-open class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" />
                </div>
            </div>
        </x-card>
    </div>
    ...
    <x-card>
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 mb-6">
            <div class="relative flex-1 max-w-md">
                ...
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search MOF subcategory name or code..."
            ...
            <x-button variant="primary" size="sm" @click="$dispatch('open-mof-subcategory-drawer', { mode: 'create' })">
                <x-heroicon-o-plus class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" />
                Add MOF Subcategory
            </x-button>
        </div>

        <x-table :headers="['Subcategory Code', 'Subcategory Name', 'Parent Category', 'MOF Codes', 'Actions']">
            @forelse($this->subcategories as $sub)
                <tr class="hover:bg-zinc-50/60 dark:hover:bg-zinc-800/20 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <x-badge variant="info" pill>{{ $sub->mof_codes_count }} Codes</x-badge>
                    </td>
