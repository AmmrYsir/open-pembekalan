...
new class extends Component
{
    ...
    #[Computed]
    public function states(): \Illuminate\Pagination\LengthAwarePaginator
    {
        return State::query()
            ->when($this->search, fn ($q) => $q->where(function ($q) {
                $q->where('fullname', 'like', "%{$this->search}%")
                    ...
                    ->orWhere('capital', 'like', "%{$this->search}%");
            }))
    ...
    {
        if ($this->deletingId) {
            State::findOrFail($this->deletingId)->delete();
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <x-ui.card>
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs text-zinc-500 dark:text-zinc-400 font-medium">Total States</p>
                ...
                <div class="w-10 h-10 rounded-xl bg-emerald-100 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 flex items-center justify-center">
                    <x-heroicon-o-map-pin class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" />
                </div>
            </div>
        </x-ui.card>
    </div>
    ...
    <x-ui.card>
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 mb-6">
            <div class="relative flex-1 max-w-md">
                ...
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search state, shortname, code..."
            ...
            <x-ui.button variant="primary" size="sm" @click="$dispatch('open-state-drawer', { mode: 'create' })">
                <x-heroicon-o-plus class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" />
                Add New State
            </x-ui.button>
        </div>

        <x-ui.table :headers="['State Code', 'Short Name', 'Full Name', 'Capital', 'Actions']">
            @forelse($this->states as $st)
                <tr class="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors">
                    <td class="px-6 py-4 whitespace-nowrap font-mono text-xs font-semibold text-zinc-900 dark:text-zinc-100">
                        {{ $st->code }}
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center gap-2">
                            <x-ui.button variant="outline" size="sm" @click="$dispatch('open-state-drawer', { mode: 'view', id: {{ $st->id }} })">
                                View
                            </x-ui.button>
                            <x-ui.button variant="outline" size="sm" @click="$dispatch('open-state-drawer', { mode: 'edit', id: {{ $st->id }} })">
                                Edit
                            </x-ui.button>
                            <x-ui.button variant="outline" size="sm" class="text-rose-600 dark:text-rose-400 hover:border-rose-300" wire:click="confirmDelete({{ $st->id }})">
                                Delete
                            </x-ui.button>
                        </div>
                    </td>
    ...
    @if($showDeleteConfirm)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/40 backdrop-blur-xs">
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 max-w-sm w-full space-y-4 shadow-xl">
                ...
                <p class="text-sm text-zinc-600 dark:text-zinc-400">Are you sure you want to delete this state reference?</p>
                <div class="flex justify-end gap-3 pt-2">
                    <x-ui.button variant="outline" size="sm" wire:click="cancelDelete">Cancel</x-ui.button>
                    <x-ui.button variant="primary" size="sm" class="bg-rose-600 hover:bg-rose-700 text-white border-transparent" wire:click="delete">Delete State</x-ui.button>
                </div>
            </div>
        </div>
    @endif
