...
new class extends Component
{
    ...
    #[Computed]
    public function subagencies(): \Illuminate\Pagination\LengthAwarePaginator
    {
        return Subagency::query()
            ->with('agency')
    ...
    {
        if ($this->deletingId) {
            Subagency::findOrFail($this->deletingId)->delete();
            session()->flash('success', 'Subagency deleted successfully.');
        }
    ...
    <x-ui.card>
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 mb-6">
            <div class="relative flex-1 max-w-md">
                ...
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search subagency name or code..."
            ...
            @forelse($this->subagencies as $subagency)
                <tr class="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <x-ui.badge variant="{{ $subagency->is_active ? 'success' : 'secondary' }}">
                            {{ $subagency->is_active ? 'Active' : 'Inactive' }}
                        </x-ui.badge>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center gap-2">
                            <x-ui.button variant="outline" size="sm" @click="$dispatch('open-subagency-drawer', { mode: 'view', id: {{ $subagency->id }} })">
                                View
                            </x-ui.button>
                            <x-ui.button variant="outline" size="sm" @click="$dispatch('open-subagency-drawer', { mode: 'edit', id: {{ $subagency->id }} })">
                                Edit
                            </x-ui.button>
                            <x-ui.button variant="outline" size="sm" class="text-rose-600 dark:text-rose-400 hover:border-rose-300" wire:click="confirmDelete({{ $subagency->id }})">
                                Delete
                            </x-ui.button>
                        </div>
                    </td>
    ...
    @if($showDeleteConfirm)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/40 backdrop-blur-xs">
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 max-w-sm w-full space-y-4 shadow-xl">
                ...
                <p class="text-sm text-zinc-600 dark:text-zinc-400">Are you sure you want to delete this subagency?</p>
                <div class="flex justify-end gap-3 pt-2">
                    <x-ui.button variant="outline" size="sm" wire:click="cancelDelete">Cancel</x-ui.button>
                    <x-ui.button variant="primary" size="sm" class="bg-rose-600 hover:bg-rose-700 text-white border-transparent" wire:click="delete">Delete Subagency</x-ui.button>
                </div>
            </div>
        </div>
    @endif
