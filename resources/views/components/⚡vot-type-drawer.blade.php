...
new class extends Component
{
    ...
    public function save(): void
    {
        ...
        if ($this->mode === 'edit' && $this->activeId) {
            $vot = VotType::findOrFail($this->activeId);
            $vot->update($validated);
            session()->flash('success', 'Vot Type updated successfully.');
        } else {
            VotType::create($validated);
            session()->flash('success', 'Vot Type created successfully.');
        }

        $this->dispatch('vot-type-saved');
        $this->showPanel = false;
    }
};
?>
            ...
            <div x-show="open" x-transition:enter="transform transition ease-in-out duration-300" x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0"
                ...
                <div class="p-6 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
                    <div>
                        <h2 class="text-lg font-bold text-zinc-900 dark:text-zinc-100">
                            {{ $mode === 'create' ? 'Add Vot Type' : ($mode === 'edit' ? 'Edit Vot Type' : 'Vot Type Details') }}
                        </h2>
                        <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">Budget vot classification master code.</p>
                    </div>
                ...
                <div class="flex-1 overflow-y-auto p-6 space-y-4">
                    @if($mode === 'view')
                        <div class="space-y-4 text-sm">
                            <div>
                                <label class="text-xs font-medium text-zinc-500 dark:text-zinc-400">Vot Code</label>
                                <p class="font-mono text-xs font-bold text-emerald-700 dark:text-emerald-400 mt-0.5">{{ $code }}</p>
                            </div>
                            <div>
                                <label class="text-xs font-medium text-zinc-500 dark:text-zinc-400">Description / Account Name</label>
                    ...
                    @else
                        <form wire:submit.prevent="save" class="space-y-4">
                            <div>
                                <x-ui.label for="vot_code">Vot Code *</x-ui.label>
                                <x-ui.input id="vot_code" wire:model="code" placeholder="e.g. B21" />
                                @error('code') <p class="text-xs text-rose-500 mt-1">{{ $message }}</p> @enderror
                            </div>

                            <div>
                                <x-ui.label for="vot_name">Description / Account Name *</x-ui.label>
                                <x-ui.input id="vot_name" wire:model="name" placeholder="e.g. Bekalan dan Bahan Mentah" />
                ...
                <div class="p-6 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 flex justify-end gap-3">
                    @if($mode === 'view')
                        ...
                        <x-ui.button variant="primary" size="sm" wire:click="switchToEdit">Edit Vot Type</x-ui.button>
                    @else
                        <x-ui.button variant="outline" size="sm" wire:click="closePanel">Cancel</x-ui.button>
                        <x-ui.button variant="primary" size="sm" wire:click="save">Save Vot Type</x-ui.button>
                    @endif
                </div>
            </div>
        </div>
