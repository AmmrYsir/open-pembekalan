...
new class extends Component
{
    ...
    #[Computed]
    public function votTypes(): \Illuminate\Pagination\LengthAwarePaginator
    {
        return VotType::query()
    ...
    {
        if ($this->deletingId) {
            VotType::findOrFail($this->deletingId)->delete();
            session()->flash('success', 'Vot Type deleted successfully.');
        }
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <x-ui.card>
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs text-zinc-500 dark:text-zinc-400 font-medium">Total Vot Types</p>
                ...
                <div class="w-10 h-10 rounded-xl bg-emerald-100 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 flex items-center justify-center">
                    <x-heroicon-o-banknotes class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" />
                </div>
            </div>
        </x-ui.card>
    </div>
    ...
    <x-ui.card>
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 mb-6">
            <div class="relative flex-1 max-w-md">
                ...
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search Vot code or description..."
            ...
            <x-ui.button variant="primary" size="sm" @click="$dispatch('open-vot-type-drawer', { mode: 'create' })">
                <x-heroicon-o-plus class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" />
                Add Vot Type
            </x-ui.button>
        </div>

        <x-ui.table :headers="['Vot Code', 'Description / Account Name', 'Actions']">
            @forelse($this->votTypes as $vot)
                <tr class="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center gap-2">
                            <x-ui.button variant="outline" size="sm" @click="$dispatch('open-vot-type-drawer', { mode: 'view', id: {{ $vot->id }} })">
                                View
                            </x-ui.button>
                            <x-ui.button variant="outline" size="sm" @click="$dispatch('open-vot-type-drawer', { mode: 'edit', id: {{ $vot->id }} })">
                                Edit
                            </x-ui.button>
                            <x-ui.button variant="outline" size="sm" class="text-rose-600 dark:text-rose-400 hover:border-rose-300" wire:click="confirmDelete({{ $vot->id }})">
                                Delete
                            </x-ui.button>
                        </div>
                    </td>
    ...
    @if($showDeleteConfirm)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/40 backdrop-blur-xs">
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 max-w-sm w-full space-y-4 shadow-xl">
                ...
                <p class="text-sm text-zinc-600 dark:text-zinc-400">Are you sure you want to delete this vot type code?</p>
                <div class="flex justify-end gap-3 pt-2">
                    <x-ui.button variant="outline" size="sm" wire:click="cancelDelete">Cancel</x-ui.button>
                    <x-ui.button variant="primary" size="sm" class="bg-rose-600 hover:bg-rose-700 text-white border-transparent" wire:click="delete">Delete Vot Type</x-ui.button>
                </div>
            </div>
        </div>
    @endif
